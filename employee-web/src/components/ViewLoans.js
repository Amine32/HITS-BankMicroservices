...
function ViewLoans() {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/loan/api/loans/all')
            .then(response => {
        ...
        axios.get('http://localhost:8080/loan/api/rates/all')
            .then(response => {
    ...
    return (
        <div>
                        ...
                        <ListGroup variant="flush">
                            {loans.map(loan => (
                                <ListGroup.Item key={loan.id}>
                                    ...
                                    <Button variant="primary" onClick={() => setSelectedLoan(loan)}
                                            style={{float: 'right'}}>
                                        View Details
                                    </Button>
                                </ListGroup.Item>
            ...
            <CreateRate show={showCreateRateModal} onHide={() => setShowCreateRateModal(false)}/>
            <EditRate rate={selectedRate} show={showEditRateModal} onHide={() => setShowEditRateModal(false)}/>
        </div>
    );
}
